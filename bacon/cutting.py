...
class CuttingBoard(object):
	...
	def _fill_slice(self, slice, query, dataset):
		...
		slice._data = root
		logger.debug("NEW: slice %s for query %s"
			% (slice._ident, query.__dict__))
		return slice
	...
	def _get_cached_slice(self, qnew):
		...
		logger.debug("HIT: slice %s created from %s by %s for query %r"
	...
	def _cache_slice(self, slice):
		if len(self._slices) > 20:  # TODO: make this configurable
			...
			logger.debug("PURGED: slice %s for query %r", old._ident, old.query.__dict__)
...
class DrillOnFirstAxis(SliceReuseStrategy):
	...
	def is_compatible(self, slice):
		...
		if not qold.axes or qold.axes[0] != label or op != 'eq':
			logger.debug("drill NOMATCH: new filter not on first axis: %r", qold.__dict__)
			return False
...
class ManipulateSlice(SliceReuseStrategy):
	...
	def _make_filter_predicate(self, slice):
		...
		""" % {
			...
			'ps': ' and '.join('op%d(key[idx%d], v%d)' % (i, i, i)
				for i in xrange(L)), }), d
		)
		p = d['p']
...
class Slice(object):
	...
	def __repr__(self):
		return '<%s dim=%s at 0x%08X>' % (
			self.__class__.__name__, self.dim, id(self))
	...
	def __getitem__(self, idx):
		...
		raise KeyError(
			"can't slice further a 0 dimension slice: use '.record' instead")
	...
	def iter_record(self):
		record = self.record
		return (LabeledValue(l, record[l.name].get(), record=record)
	...
	def record(self):
		...
		raise AttributeError(
			'record not available on a %d dimension slice' % self.dim)
...
class LabeledValue(object):
	...
	def __repr__(self):
		return '%s(value=%r, label=%r, record=%r)' % (
			...
			self.value, self.label, self.record)
...
def _make_acc_function(query, cubedef):
	...
	def zero_f(%(accs)s):
		return {%(items)s}
	""" % {
		'accs': ", ".join("a%d=labels[%d].acc" % (i, i)
			for i in xrange(len(labels))),
		'items': ", ".join("%r: a%d()" % (label.name, i)
...
%(adds)s
	""" % {
			'es': ", ".join("e%d=labels[%d].extract" % (i, i)
				for i in xrange(len(labels))),
			'adds': "\n".join("\t\tacc[%r].add(e%d(record), record)"
				% (label.name, i) for i, label in enumerate(labels)),
		}), d)
		acc_f = d['acc_f']
	else:
...
_op_map = {
	...
	'equals': hasonly, 'notequals': lambda a, b: not hasonly(a, b),
	'match': ismatch, 'nmatch': lambda a, b: not ismatch(a, b),
}
...
def _make_filter_predicate(query, cubedef):
	...
	""" % {
		...
		'vs': ', '.join('v%d=vs[%d]' % (i, i) for i in xrange(L)),
		'ops': ', '.join('op%d=ops[%d]' % (i, i) for i in xrange(L)),
		'ps': ' and '.join('op%d(e%d(record), v%d)' % (i, i, i)
			for i in xrange(L)), }), d)
	p = d['p']
