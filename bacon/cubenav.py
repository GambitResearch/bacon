...
CurrentFilter = namedtuple('CurrentFilter', """
	name pretty_name
	op pretty_op
...
class Navigator(object):
	...
	def iter_expansions(self):
		...
		labels.sort(key=lambda l: (l.dimension or u'\uffff', l.rank))
	...
	def _iter_filters(self, filters):
		...
		for name, op, value in filters:
			...
			yield CurrentFilter(name=name, op=op, value=value, str_value=text_type(value),
	...
	_pretty_op = {
		...
		'gt': '>', 'ge': u"\u2265",
		'lt': '<', 'le': u"\u2264",
		'in': 'is any of:', 'ni': 'is none of:',
		'hasall': 'has all of:', 'hasnone': 'has none of:',
		'hasany': 'has any of:', 'hasonly': 'has only:',
	}
	...
	def order_url(self, label, lvs=()):
		...
		if order:
			...
			if odir != '-' and olabel == label:
				if ((not lvs and not ovals) or
						list(ovals) == [lv.value for lv in lvs]):
					return None
