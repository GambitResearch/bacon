...
class CubeDef(object):
	...
	def add_label(self, label):
		...
		if not isinstance(label, Label):
			raise TypeError(
				"expected 'Label' instance, %r got instead" % label)
	...
	def add_measure(self, measure):
		...
		if not isinstance(measure, Label):
			raise TypeError(
				"expected 'Label' instance, %r got instead" % measure)
	...
	def add_hierarchy(self, name_from, name_to):
		...
		if l1.dimension is None and l2.dimension is None:
			...
			raise errors.DataError(
				...
				"and %s (on label %s)" %
				(l1.dimension, l1, l2.dimension, l2))
...
class Field(object):
	...
	def __repr__(self):
		return "<%s %r at 0x%08X>" % (
			type(self).__name__, self._name, id(self))
...
class Label(object):
	...
	"""
	def __init__(self, name_or_field, extract=None, acc=None,
			title=None, pretty=None,
			parse=None, unparse=None,
			...
			child_of=None, parent_of=None, dimension=None, cls='label',
			allow_pivot=True,
			hidden=False):
		...
		if isinstance(name_or_field, Field):
			...
			self.field = Field(name=name_or_field,
				extract=extract, title=title, pretty=pretty)
	...
	sql_opmap = {
		'ge': '>=', 'gt': '>', 'le': '<=', 'lt': '<',
		'eq': '=', 'ne': '<>', 'in': 'in', 'ni': 'not in',
		'match': '~', 'nmatch': '!~',
	}

	django_opmap = {
		'ge': '__gte', 'gt': '__gt', 'le': '__lte', 'lt': '__lt',
		'eq': '', 'ne': '', 'in': '__in', 'ni': '__in',
		'match': '__regex', 'nmatch': '__regex',
		'contains': '__contains', 'icontains': '__icontains',
	}
	...
	def add_q_filter(self, queryset, op, value):
		...
		if type(self.django_expression) == str:
			if op not in self.django_opmap:
				raise NotImplementedError("The operation %s is not implemented as a django query filter" % op)
	...
	def add_sql_filter(self, sql, op, value):
		...
		if isinstance(value, frozenset):
			...
			if None in value:
				...
				if op == 'in':
					filter = ("(((%s) is null) or %s)"
						% (self.sql_expression, filter))
...
class NullableLabel(Label):
	...
	def __init__(self, name, none_value='', none_label=u'(none)', **kwargs):
	...
	def get_filter(self, op, value):
		if op in ('hasany', 'intersects'):
			...
			if value == '{}':
				...
				return "(((%s) @> (%%s)) and ((%s) <@ (%%s)))" % (self.field.name,
...
class BoolTypeLabel(Label):
	...
	def unparse(self, v,
			_f={False: '0', True: '1', None: ''}):
		return _f[v]

	def pretty(self, v, record=None,
			_f={False: u'No', True: u'Yes', None: u'Unknown'}):
		return _f[v]
...
class DatetimeTypeLabel(DatetimeDateTypeLabel):
	...
	def parse(self, s):
		...
		if (datetime_obj.tzinfo is None or
				datetime_obj.tzinfo.utcoffset(datetime_obj) is None):
...
class DateTruncLabel(DateHierarchyLabel, DatetimeDateTruncLabelMixin):
	@property
	def sql_expression(self):
		return "date_trunc('%s', %s)::date" % (self.SQL_DATE_FIELD, self._sql_expression)
...
class QuarterLabelMixin(object):
	...
	def pretty(self, d, record=None):
		if not d:
			return 'Unknown'
		return b'Q%d\xA0%d'.decode('latin1') % (
...
class Measure(Label):
	...
	def add_sql_measure(self, sql):
		return self.acc.manipulate_sql(sql,
			self.name, self.sql_expression)
...
class AttributeRatioMeasure(AttributeMeasure):
	def __init__(self, name, attr_num, attr_denom,
			expr_num=None, expr_denom=None, **kwargs):
		if 'acc' not in kwargs:
