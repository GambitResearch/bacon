...
def _render_widget(widget, panel):
	type_name = re.sub(r'([a-z])([A-Z])',
			lambda m: '%s_%s' % m.groups(),
		widget.__class__.__name__.replace('Widget', '')).lower()
...
def _render_DatesRangeWidget(rv, widget, panel):
	rv['urls'] = OrderedDict(zip(
		('no_value', 'from_only', 'to_only', 'both_values'),
		widget.get_urls(panel)))

	f, t = panel.get_query().get_range(widget.axis)
	rv['values'] = {
		'from': str(f) if f else None,
		'to': str(t) if t else None}
...
def render_table_json(table):
	...
	if table.get_query().pivot:
		...
		rv['pivots'], rv['columns'], rv['rows'] = \
			_render_table_pivot(rtable, links)
	else:
