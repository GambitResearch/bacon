...
class Styles(object):
	...
	money = xlwt.easyxf('align: horiz right', num_format_str=u'#,##0.00')
	price = xlwt.easyxf('align: horiz right', num_format_str=u'#,##0.000')
	discrepancy = xlwt.easyxf('align: horiz right',
		num_format_str=u'\u00A3#,##0.00;[Red]-\u00A3#,##0.00;0.00;"TEXT"')
	discrepancy_percentage = xlwt.easyxf('align: horiz right',
		num_format_str=u'#,##0.000%')
...
class WSWrapper(object):
	...
	def write_merge(self, colspan, data, rowspan=1, **kwargs):
		...
		"""
		self.ws.write_merge(
			self.row, self.row + rowspan - 1,
			self.col, self.col + colspan - 1,
			data, **kwargs)
		if colspan == 1:
			self._update_width(data)
		self.col += colspan
