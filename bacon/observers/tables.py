...
class PaginatedViewer(Viewer):
	...
	def pages(self, nrows):
		...
		return chain(
			[(u"\xab\xa0Prev", curpage - 1 if curpage else None, False)],
			run(0, curpage),
			[label(str(curpage + 1), True)],
			run(curpage + 1, npages),
			[(u"Next\xa0\xbb", curpage + 1 if curpage < npages - 1 else None,
				False)],
		)
	...
	def to_string_page(self, n):
		...
		params = {'': ":".join(str(x) if x is not None else ""
			for x in (limit, offset, nrows))}
...
class Table1D(UrlMaker, BaseTableRenderer):
	...
	def totals(self):
		...
		if self._nrows > 1:
			return (LabeledValue(l, tots[l.name].get(), record=tots)
				for l in self.value_titles())
		else:
			return ()
...
class TablePivot(UrlMaker, BaseTableRenderer):
	...
	def rows(self):
		self._totals = col_totals = [self.slice.make_acc()
			for i in self.pivot_lvs()]
