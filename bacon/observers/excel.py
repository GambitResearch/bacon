...
def render_table_pivot(ws, table):
    ...
    for i, (pivot_label, pivot_lvs) in enumerate(table.pivot_titles()):
        ws.write_merge(
            len(table.label_titles()), ensure_unicode(pivot_label), style=style_title
        )
        for label in pivot_lvs:
            ws.write_merge(
                len(table.value_titles()), ensure_unicode(label), style=style_title
            )
        if i == 0:
            ws.write_merge(
                len(table.value_titles()),
                "Total",
