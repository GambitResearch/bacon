...
class Sum(Accumulator):
	...
	def __repr__(self):
		return "<%s (%r) at 0x%08X>" % (
			self.__class__.__name__, self.acc, id(self))
...
class Average(Accumulator):
	...
	def __repr__(self):
		return "<%s (%r items) at 0x%08X>" % (
			self.__class__.__name__, self.n, id(self))
...
class Group(Accumulator):
	...
	def __repr__(self):
		return "<%s (%r) at 0x%08X>" % (
			self.__class__.__name__, self.val, id(self))
	...
	class LabeledAcc_(Accumulator):
		...
		def __repr__(self):
			return "<%s (%r, %r) at 0x%08X>" % (
				"LabeledAcc", self.label, self.acc, id(self))
...
def RatioSum(attr_num, attr_denom, expr_num=None, expr_denom=None):
	class RatioSum_(Accumulator):
		...
		def __repr__(self):
			return "<%s (%r) at 0x%08X>" % (
				self.__class__.__name__, self.accs, id(self))

		@classmethod
		def manipulate_sql(self, sql, column, expression):
			sql = sql.add_aggregate(attr_num,
				"sum(%s)" % (expr_num or attr_num))
			sql = sql.add_aggregate(attr_denom,
				"sum(%s)" % (expr_denom or attr_denom))
			return sql
